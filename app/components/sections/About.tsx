    ...
    <section id="about" className="relative overflow-hidden px-5 py-24 sm:px-8">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-end">
          <Reveal>
            <p className="font-mono text-sm uppercase tracking-[0.28em] text-cyan-200/80">
              developer with product instincts
            </p>
            <h3 className="mt-5 max-w-3xl text-balance text-4xl font-semibold leading-tight text-white sm:text-5xl">
              I build the kind of web experiences that feel calm on the
              surface and disciplined underneath.
            </h3>
          </Reveal>
          ...
          <Reveal delay={0.1}>
            <div className="space-y-5 border-l border-white/10 pl-6">
              {focusLines.map((line, index) => (
                <motion.p
                  key={line}
                  className="relative text-lg leading-9 text-zinc-300"
                  initial={{ opacity: 0, x: 24 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.6, delay: index * 0.09 }}
                >
                  <span className="absolute -left-[1.73rem] top-3 size-2 rounded-full bg-cyan-300 shadow-[0_0_16px_rgba(103,232,249,0.7)]" />
                  {line}
                </motion.p>
              ))}
            </div>
        ...
        <Reveal className="mt-14">
          <div className="grid gap-8 border-y border-white/10 py-7 sm:grid-cols-3">
            {stats.map((item, index) => (
              ...
              >
                ...
                <span className="max-w-36 text-sm leading-5 text-zinc-500">
                  {item.label}
                </span>
              </motion.div>
            ))}
        ...
        <Reveal className="mt-20">
          ...
          <div className="relative hidden min-h-80 lg:block">
            ...
            {workflow.map((step, index) => {
              ...
              return (
                <motion.div
                  key={step.title}
                  className={`absolute max-w-56 ${positions[index]}`}
                ...
                >
                  <div className="mb-4 flex items-center gap-3">
                    <span className="flex size-12 items-center justify-center rounded-full border border-cyan-300/40 bg-[#0d1117] font-mono text-sm text-cyan-200 shadow-[0_0_34px_rgba(34,211,238,0.12)]">
          ...
          <div className="relative grid gap-8 border-l border-cyan-300/25 pl-7 lg:hidden">
            {workflow.map((step, index) => (
              <motion.div
                key={step.title}
                className="relative"
                ...
                transition={{ duration: 0.55, delay: index * 0.08 }}
              >
                <span className="absolute -left-[2.15rem] top-1 flex size-7 items-center justify-center rounded-full border border-cyan-300/40 bg-[#0d1117] font-mono text-[0.65rem] text-cyan-200">
                  {String(index + 1).padStart(2, "0")}
                </span>
                <h4 className="text-xl font-semibold text-white">
                  {step.title}
                </h4>
                <p className="mt-2 leading-7 text-zinc-400">{step.copy}</p>
              </motion.div>
            ))}
          </div>
        </Reveal>
